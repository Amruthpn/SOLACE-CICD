...
How it works
You drop a JSON file into configs/, named with a CREATE_ or MODIFY_ prefix.
You push (or commit directly on GitHub) to main.
Jenkins checks out the repo and runs Ansible against the target broker.
deploy_solace.yml scans configs/*.json, and for each file loops into inner_provision.yml, which reads the filename prefix to decide whether to POST (create) or PATCH (modify) via the Solace SEMP v2 REST API.
SOLACE-CICD/
...
CREATE_*.json → POSTs a new queue (and binds any listed topic subscriptions).
MODIFY_*.json → PATCHes an existing queue's parameters.
Jenkins setup
Manage Jenkins → Credentials → add a Username with password credential with ID exactly solace_admin_creds (your Solace SEMP admin user/pass).
New Item → Pipeline → Pipeline script from SCM → Git → this repo URL → Script Path: Jenkinsfile.
Click Build Now. The pipeline runs against Dev (inventory/dev.ini, port 8080) then Prod (inventory/prod.ini, port 8081).
Local broker ports
